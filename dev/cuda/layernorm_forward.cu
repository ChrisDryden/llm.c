...
__global__ void layernorm_forward_kernel10(floatX* __restrict__ out, floatX* __restrict__ mean, floatX* __restrict__ rstd,
                                    const floatX*  __restrict__ inp, const floatX*  __restrict__ weight,
                                    const floatX* __restrict__ bias, int N, int C) {
    int lane_id = threadIdx.x % WARP_SIZE;
    int warp_id = threadIdx.x / WARP_SIZE;
...
void layernorm_forward7(floatX* out, floatX* mean, floatX* rstd,
    ...
    const int N = B * T;
    const int grid_size = N;
    //const int grid_size = ceil_div((int)(N*WARP_SIZE), block_size*x128::size);
...
void layernorm_forward8(floatX* out, floatX* mean, floatX* rstd,
    ...
    const int N = B * T;
    const int grid_size = N;
    //const int grid_size = ceil_div(N, x128::size);
...
void layernorm_forward10(floatX* out, floatX* mean, floatX* rstd,
                       const floatX* inp, const floatX* weight, const floatX* bias,
                       int B, int T, int C,
                       const int block_size) {
    assert(block_size % 32 == 0);
    ...
    layernorm_forward_kernel10<<<grid_size, block_size>>>(out, mean, rstd, inp, weight, bias, N, C);
    cudaCheck(cudaGetLastError());
}
...
void layernorm_forward(int kernel_num,
    ...
    switch (kernel_num) {
        ...
        case 9:
            ...
            break;
        case 10:
            layernorm_forward10(out, mean, rstd, inp, weight, bias, B, T, C, block_size);
            break;
        default:
